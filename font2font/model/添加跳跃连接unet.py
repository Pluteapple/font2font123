...
class UNet(object):
    ...
    def decoder(self, encoded, encoding_layers, ids, inst_norm, is_training, reuse=False):
        with tf.variable_scope("generator"):
            ...
            def SPADE(x,s,output_width,layer):
                x =conv2d(x, output_filters=output_width*2, kh=1, kw=1, sh=1, sw=1, scope="SPADE%d_conv1" % layer)
                a=conv2d(s, output_filters=output_width*2, kh=1, kw=1, sh=1, sw=1, scope="SPADE%d_conv2" % layer)
                b=conv2d(s, output_filters=output_width*2, kh=1, kw=1, sh=1, sw=1, scope="SPADE%d_conv3" % layer)
                return tf.multiply(x,(1.0 + a)) + b
            ...
            def decode_layer(x, output_width, output_filters, layer, enc_layer, dropout=False, do_concat=True):
                ...
                if do_concat:
                    dec=SPADE(dec,enc_layer,output_filters,layer)
    ...
    def generator(self, images, embeddings, embedding_ids, inst_norm, is_training, reuse=False):
        ...
        return output,e8,tf.nn.tanh(output)
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        g_loss = cheat_loss + l1_loss*2.0+self.Lcategory_penalty * fake_category_loss + const_loss + tv_loss
        if no_target_source:
