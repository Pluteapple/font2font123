...
KR_CHARSET = None
####################改#######################
#DEFAULT_CHARSET = "./charset/text657.json"
DEFAULT_CHARSET = "./charset/text1077.json"
...
def draw_example(ch, src_font, dst_font, canvas_size, x_offset, y_offset, filter_hashes):
    dst_img = draw_single_char(ch, dst_font, canvas_size, 10,-55)#图左边的 
    # check the filter example in the hashes or not
    dst_hash = hash(dst_img.tobytes())
    if dst_hash in filter_hashes:
        return None
    src_img = draw_single_char(ch, src_font, canvas_size, 10, 10)#图右边的
...
def font2img(src, dst, charset, char_size, canvas_size,
    ...
    src_font = ImageFont.truetype(src, size=220) #图右边的字体
    dst_font = ImageFont.truetype(dst, size=250) #图左边的字体
