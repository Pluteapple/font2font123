...
def draw_example(ch, src_font, dst_font, canvas_size, x_offset, y_offset, filter_hashes):
    dst_img = draw_single_char(ch, dst_font, canvas_size, 0, 0)
    # check the filter example in the hashes or not
    dst_hash = hash(dst_img.tobytes())
    if dst_hash in filter_hashes:
        return None
    #src_img = draw_single_char(ch, src_font, canvas_size, 0, 0)
...
def font2img(src, dst, charset, char_size, canvas_size,
    ...
    src_font = ImageFont.truetype(src, size=250)
    dst_font = ImageFont.truetype(dst, size=230)
    filter_hashes = set()
    if filter_by_hash:
    ...
    for c in charset:
        ...
        if e:
            name = json.dumps("uni%s" % c)
            e.save(os.path.join(sample_dir, "%d_%s.jpg" % (label, name[1:4]+name[6:-1])))
            count += 1
            if count % 100 == 0:
...
#python font2img.py --src_font ./font/src.ttf  --dst_font ./font/NewFont-Regular.ttf  --charset=CN --sample_count=300 --sample_dir=./experiment/testimg/train  --label=0 --filter=0 --shuffle=0
