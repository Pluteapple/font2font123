...
from .ops import conv2d, deconv2d, lrelu, fc, batch_norm, init_embedding, conditional_instance_norm,conv,max_pooling,hw_flatten
...
EvalHandle = namedtuple("EvalHandle", ["encoder", "generator", "target", "source", "embedding","tempfake_B"])
...
class UNet(object):
    ...
    def SKConv(self,input, M, r, L=32, stride=1, is_training=True):
    ...
    def decoder(self, encoded, encoding_layers, ids, inst_norm, is_training, reuse=False):
        with tf.variable_scope("generator"):
            ...
            # pam = self.PAM(d5)
            # pam = Conv2D(s8, 3, padding='same', use_bias=False, kernel_initializer='he_normal')(pam)
            ...
            #
            # cam = self.CAM(d5)
            # cam = Conv2D(s8, 3, padding='same', use_bias=False, kernel_initializer='he_normal')(cam)
            ...
            out = self.SKConv(d7, 3, 2, is_training)
            dec = tf.concat([out, d7], 3)
            d8 = decode_layer(dec, s, self.output_filters, layer=8, enc_layer=None, do_concat=False)
            print('d7')
            print(d7)
            ...
            print("out"*10)
            print(out)
            self.decoderlayer["d7"] = d7
            self.decoderlayer["d6"] = d6



            #out = Conv2D(3, 3, padding='same', use_bias=False, kernel_initializer='he_normal')(feature_sum)
            #output = tf.nn.tanh(feature_sum)  # scale to (-1, 1)
            return d8



    def NewSRLayer(self,x):
        with tf.variable_scope("SRLayer"):
            conv1 = tf.nn.relu(conv2d(x,output_filters=64, kh=9,kw=9,sh=1,sw=1,scope="SR%d_conv" % 1))
            conv2 = tf.nn.relu(conv2d(conv1, output_filters=32, kh=5,kw=5,sh=1,sw=1,scope="SR%d_conv" % 2))
            conv3 =conv2d(conv2, output_filters=16, kh=3,kw=3,sh=1,sw=1,scope="SR%d_conv" % 3)
            conv4 = conv2d(conv3, output_filters=3, kh=1, kw=1, sh=1, sw=1, scope="SR%d_conv" % 4)
            tensor = tf.add(x, conv4)
            return  tf.nn.tanh(tensor),conv4  # scale to (-1, 1)


    def SFTLayer(self,x,output_filters):

        conv = conv2d(x, output_filters=output_filters/2,kh=1,kw=1,sh=1,sw=1,scope="sft_scale%d_conv" % 1)
        act = tf.nn.leaky_relu(conv, alpha=0.1)
        scale = conv2d(act, output_filters=output_filters, kh=1, kw=1, sh=1, sw=1, scope="sft_scale%d_conv" % 2)

        conv = conv2d(x, output_filters=output_filters/2, kh=1, kw=1, sh=1, sw=1, scope="sft_shift%d_conv" % 1)
        act = tf.nn.leaky_relu(conv, alpha=0.1)
        shift = conv2d(act, output_filters=output_filters, kh=1, kw=1, sh=1, sw=1, scope="sft_shift%d_conv" % 2)
        return x * (scale + 1) + shift

    # def Channel_ttention_module(self):
    #     batch_size, height, width, num_channels = x.get_shape().as_list()
    #     proj_query = x.view(m_batchsize, C, -1)
    #     proj_query=tf.reshape(x,batch_size,shape=[batch_size, height, width, num_channels//2])
    #
    #     def forward(self, x):
    #         """
    #             inputs :
    ...
    def generator(self, images, embeddings, embedding_ids, inst_norm, is_training, reuse=False):
        ...
        out = self.decoder(embedded,enc_layers, embedding_ids, inst_norm, is_training=is_training, reuse=reuse)
        output,temp=self.NewSRLayer(out)
        return output,e8,tf.nn.tanh(temp)

    def discriminator(self, image, is_training, reuse=False):
        with tf.variable_scope("discriminator"):
            if reuse:
                tf.get_variable_scope().reuse_variables()
            h0 = lrelu(conv2d(image, self.discriminator_dim/2, scope="d_h0_conv"))
            print('h0')
            print(h0)
            #h0 = tf.concat([h0, self.decoderlayer["d7"]], 3)
            #temp = self.google_attention(h0, 64, scope='self_d_attention')
            h1 = lrelu(batch_norm(conv2d(h0, self.discriminator_dim, scope="d_h1_conv"),is_training, scope="d_bn_1"))
            #h1 = tf.concat([h1, self.decoderlayer["d6"]], 3)
            print('h1')
            print(h1)
            temp = self.google_attention(h1, 64, scope='self_d_attention')
            h2 = lrelu(batch_norm(conv2d(temp, self.discriminator_dim*4 , scope="d_h2_conv"),is_training, scope="d_bn_2"))
            print('h2')
            print(h2)
            print('temp')
            #print(temp)
            h3 = lrelu(batch_norm(conv2d(h2, self.discriminator_dim * 8, sh=1, sw=1, scope="d_h3_conv"),is_training, scope="d_bn_3"))
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        fake_B, encoded_real_A,tempfake_B= self.generator(real_A, embedding, embedding_ids, is_training=is_training,
                                                inst_norm=inst_norm)
        ...
        newl1_loss=self.L1_penalty * tf.reduce_mean(tf.abs(fake_B - real_B))
        #mmssim_loss=self.MultiScaleSSIM(fake_B.numpy(),real_B.numpy())
        #print(mmssim_loss)
        mmssim_loss=1 - tf.image.ssim(fake_B, real_B, max_val=1.0)[0]

        batch_size, height, width, num_channels = fake_B.get_shape().as_list()
        newLoss=tf.count_nonzero(tf.abs(fake_B - real_B),dtype='float32')/tf.count_nonzero(tf.ones([batch_size, height, width, num_channels]),dtype='float32')
        print('W'*100)
        print(newLoss)
        ...
        g_loss = cheat_loss + l1_loss*2.0+mmssim_loss*100*0.9*0+ self.Lcategory_penalty * fake_category_loss + const_loss + tv_loss
        if no_target_source:
            ...
            no_target_B, encoded_no_target_A,_= self.generator(no_target_A, embedding, no_target_ids,
    ...
    def generate_fake_samples(self, input_images, embedding_ids):
        ...
        return fake_images, real_images,tempfake_B, d_loss, g_loss, l1_loss

    def validate_model(self, val_iter, epoch, step):
        labels, images = next(val_iter)
        fake_imgs, real_imgs,tempfake_B,d_loss, g_loss, l1_loss = self.generate_fake_samples(images, labels)
        print("Sample: d_loss: %.5f, g_loss: %.5f, l1_loss: %.5f" % (d_loss, g_loss, l1_loss))

        merged_fake_images = merge(scale_back(fake_imgs), [self.batch_size, 1])
        merged_real_images = merge(scale_back(real_imgs), [self.batch_size, 1])
        tempfake_B=merge(scale_back(tempfake_B), [self.batch_size, 1])
        merged_pair = np.concatenate([merged_real_images, merged_fake_images], axis=1)
        merged_pair = np.concatenate([merged_pair, tempfake_B], axis=1)
    ...
    def newinfer(self, source_obj, embedding_ids, model_dir, save_dir):
        ...
        def save_imgs(imgs, imgname):
            import json
            name=json.dumps("uni%s.png" % imgname)
            print(json.dumps("uni%s.png" % imgname))
            p = os.path.join(save_dir, name[1:4]+name[6:-1])
            print(name[0])
            save_concat_images(imgs, img_path=p)
            print("generated images saved at %s" % p)
        for imgname, source_imgs in image.get_FontImage():
            labels=0
            fake_imgs = self.generate_fake_samples(source_imgs, labels)[0] #生成
            merged_fake_images = scale_back(fake_imgs)
            print(type(imgname))
            save_imgs(merged_fake_images, imgname)
...
#https://github.com/zrl4836/tensorflow_octConv/blob/master/oct_Resnet_unit.py
#https://zhuanlan.zhihu.com/p/68120231
